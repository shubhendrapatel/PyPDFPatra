...
* **`display`**: `block`, `inline`, `inline-block` (CSS 2.1), `none`
...
* `none`
* `hidden`
* `inset` (Creates a simulated 3D recessed shadow effect, heavily used for HTML forms)
* `outset` (Creates a simulated 3D raised button effect, heavily used for HTML forms)
*(Note: `dashed`, `dotted`, and `double` will currently fall back to standard solid block rendering in the PDF scalar until vector drawing for dashed lines is fully implemented).*

## Typography & Text (CSS 1 & CSS 2.1)
* **`font-family`** (CSS 1): Standard PDF metrics (Courier, Helvetica/Arial, Times)
...
* **`text-align`** (CSS 1): `left` (MVP; `center` and `right` coming soon)
